...
void UAC_PlayerAttributes::IncreaseMaximumHealth(int32 Amount) {
	CurrentHealth += Amount;
	MaxHealth += Amount;
}

void UAC_PlayerAttributes::DecreaseMaximumHealth(int32 Amount) {
	if (CurrentHealth - Amount >= 1 && MaxHealth - Amount >= 1) {
		CurrentHealth -= Amount;
		MaxHealth -= Amount;
	}
	else {
...
//-------------------------------------------------------------------------------
//Take Melee Damage
void UAC_PlayerAttributes::CalculateIncomingMeleeDamage(float Amount) {
